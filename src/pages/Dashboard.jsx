...
const Dashboard = () => {
  ...
  const { data: tasks = [], isLoading, isError } = useQuery({
    queryKey: ["tasks"],
    queryFn: async () => {
      const res = await fetch("https://taskgo-mariana-sr.vercel.app/added-task");
      return res.json();
    },
  });
  ...
  const editTask = useMutation({
    mutationFn: async (task) => {
      const res = await fetch(`https://taskgo-mariana-sr.vercel.app/added-task/${task._id}`, {
        method: "PUT",
  ...
  const deleteTask = useMutation({
    mutationFn: async (id) => {
      await fetch(`https://taskgo-mariana-sr.vercel.app/added-task/${id}`, { method: "DELETE" });
    },
    onSuccess: () => {
      queryClient.invalidateQueries(["tasks"]);
  ...
  const Task = ({ task }) => {
    ...
    return (
      <div
        ref={drag}
        className={`bg-blue-800 p-3 rounded-lg mb-3 cursor-pointer ${isDragging ? "opacity-50" : ""}`}
      >
        ...
        <div className="flex justify-between mt-3">
          <button
            className="px-3 py-1 rounded-full bg-green-500 text-white font-semibold shadow-lg"
            onClick={() => {
              setEditedTask(task);
              setIsModalOpen(true);
            }}
  ...
  const CategoryColumn = ({ name, tasks }) => {
    ...
    return (
      <div ref={drop} className="bg-blue-600 p-5 rounded-lg  shadow-lg min-h-[400px]">
  ...
  return (
    <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
      <div>
        ...
        {isModalOpen && (
          <div className="modal-overlay fixed inset-0 flex justify-center items-center bg-white bg-opacity-75">
            <div className="modal-content bg-gray-400 p-6 rounded-lg w-1/3">
              ...
              >
                ...
                <select name="category" defaultValue={editedTask?.category} className="w-full p-2 mt-2 border rounded bg-gray-400">
        ...
        <div className="flex justify-center mb-4">
          <button onClick={() => navigate("/add-task")} className="px-6 py-6 rounded-full bg-purple-700 hover:bg-green-500 transition text-white font-semibold shadow-lg text-lg">
            + Add Task
          </button>
        </div>
      </div>
